...
import { downloadCSV } from '../csv_utils';
...
export function CreationApp() {
    ...
    useEffect(() => {
        ...
        if ((certs) && (privateKey)) {
            if (privateKeyInputRef?.current?.value == testPrivateKey) {
                alert( "You are using a test key for signing the certificates. The certificate will not validate officially.");
            }
